...
class PreferencesBoard extends StatelessWidget {
  const PreferencesBoard({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    final settings = provider.currentVideoSettings;

    return Column(
      children: settings
          .map((setting) => _SettingTile(setting: setting))
          .expand((widget) => [widget, SizedBox(height: theme.dPanelPadding)])
          .toList()
        ..removeLast(),
    );
  }
...
class _SettingTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    switch (setting) {
      case VideoNodeOverrideSingleButton s:
        return BoardVideoSetting(
          labelText: setting.getOverrideNameString(t),
          valueText: setting.getOverrideValueString(t),
          onTap: () {
            provider.updateVideoSetting(s.copySwitched());
          },
        );

      case VideoNodeOverrideDoubleButton s:
        return BoardVideoSetting(
          labelText: setting.getOverrideNameString(t),
          valueText: setting.getOverrideValueString(t),
          onTap: () {}, // not used
          onTapLeft: () {
            provider.updateVideoSetting(
              s.copyIteratedDownwards(),
            );
          },
          onTapRight: () {
            provider.updateVideoSetting(
              s.copyIteratedUpwards(),
            );
          },
        );
    }
...
//     final List<VideoNodeOverride> settings =
